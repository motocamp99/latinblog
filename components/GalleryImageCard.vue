<template>
    <div
        ...
        <!-- Image -->
        <NuxtImg :src="image.url" :alt="image.alt || image.gallery.name"
        ...
        <div
            ...
            <NuxtLink :to="`/gallery/${image.gallery.id}/${slugify(image.gallery.name)}`"
                class="text-white font-semibold text-md hover:underline">
                {{ image.gallery.name }}
            </NuxtLink>

            <!-- Tags -->
            <div v-if="image.tags && image.tags.length" class="flex flex-wrap gap-2 mt-2">
